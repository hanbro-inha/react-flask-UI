...
def create_scc12(packer, apply_accel, enabled, cnt, scc12):
  func_state = 0.0
  if v2x_sock is not None:
    print("ee")
    data, addr = v2x_sock.recvfrom(200)
    print("ff")
    ...
    try:
      print(data)
      func_state, apply_acc, reqe = data.split('//')
      func_state = int(func_state)
      apply_acc = float(apply_acc) 
      reqe = int(reqe)
    except:
      print("split failed")
  else :
  ...
  if func_state == 1:
    apply_accel = apply_acc

  print(apply_accel)
  print("stop req :", scc12["StopReq"])
