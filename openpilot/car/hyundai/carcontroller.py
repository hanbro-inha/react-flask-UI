...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    lkas_active = enabled and abs(CS.angle_steers) < 200. and self.lkas_button
    ...
    if CS.scc_bus and self.longcontrol and frame % 2: # send scc12 to car if SCC not on bus 0 and longcontrol enabled
      can_sends.append(create_scc12(self.packer, apply_accel, enabled, self.scc12_cnt, CS.scc12, self.v2x_sock))
      self.scc12_cnt += 1
